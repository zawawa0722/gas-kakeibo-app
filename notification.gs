...
function sendEmailRequestCsvStorage() {
  ...
  GmailApp.sendEmail(
    RECIPIENTS.naoto,
    SUBJECT_REQUEST_CSV_STORAGE,
    BODY_REQUEST_CSV_STORAGE,
  );
}
  ...
  if (buttonFlug == false) {
    if (emptyCheckFlug == false) {
      if (hasWarnOrError) {
        GmailApp.sendEmail(
          RECIPIENTS.naoto,
          SUBJECT_SCCEED_IMPORT,
          BODY_SCCEED_IMPORT_INCLUDE_ERROR,
        );
      } else {
        GmailApp.sendEmail(
          RECIPIENTS.naoto,
...
function getResaltBalance(sheet, dateRange, lastMonthText) {
  ...
  // 8〜10行目の値を取得
  let values = sheet.getRange(8, lastMonthIndex + 2, 3, 1).getValues();
  ...
  for (let i = 0; i < labels.length; i++) {
    ...
    if (label === "貯蓄率") {
      value =
        isNaN(parseFloat(value)) || value === "" ? 0 : parseFloat(value) * 100; // 100を掛ける
...
function createMessage() {
  ...
  // 帳簿管理のA8~A12の項目を取得
  try {
    incomeAndExpenditure = getResaltBalance(
      tyoubosheet,
      tyoubodateRange,
      lastMonthText,
    );
